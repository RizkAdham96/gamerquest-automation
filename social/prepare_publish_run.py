...
def get_source_id(payload):
    ...
    if isinstance(selected, dict):
        source_id = str(selected.get("source_id", "")).strip()
    if not source_id:
        raise RuntimeError("No source_id found in social-output.json.")
    return source_id


def write_publish_ready_file(source_id, image_paths, carousel_version):
    ...
    if len(image_urls) != 3:
        raise RuntimeError("Exactly three public image URLs are required.")
    carousel_version = str(carousel_version or "").strip()
    ...
    with READY_FILE.open("w", encoding="utf-8") as file:
...
def main():
    ...
    source_id = get_source_id(payload)
    ...
    ready_payload = write_publish_ready_file(
        ...
        carousel_version=result["carousel_version"],
    )
    print("Carousel prepared for publishing.")
    print(f"Source ID: {result['source_id']}")
    print(f"Carousel version: {result['carousel_version']}")
    print(f"Folder: {result['folder_name']}")
    print("Images:")
    for image_path in result["image_paths"]:
        print(f"- {image_path}")
    print(f"Manifest: {result['manifest']}")
    print(f"Publish-ready file: {READY_FILE}")
    print("Public image URLs:")
    for image_url in ready_payload["image_urls"]:
        print(f"- {image_url}")
