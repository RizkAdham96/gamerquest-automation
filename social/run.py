...
def run():
    ...
    if not content:
        save_output({"status": "skipped", "reason": "no_content"})
        return
    ...
    try:
        ...
        save_output({"status": "error", "reason": "ai_generation_failed", "error": str(error)})
        return

    print(f"Candidate ideas generated: {len(ideas)}")
    if not ideas:
        save_output({"status": "skipped", "reason": "no_ideas"})
        return

    best_idea = choose_best_idea(ideas)
    if not best_idea:
        print("All ideas were rejected.")
        save_output({"status": "skipped", "reason": "repetitive_or_invalid"})
        return

    score = best_idea.get("total_score", 0)
    print(f"Best social idea score: {score}")
    if score < MINIMUM_PUBLISH_SCORE:
        save_output({"status": "skipped", "reason": "low_score", "best_score": score})
        return

    # Only the winning compact concept gets a second AI call for full slides/caption.
    try:
        complete_idea = idea_generator.expand_idea(best_idea, content)
    except Exception as error:
        print(f"AI carousel expansion failed: {error}")
        save_output({"status": "error", "reason": "carousel_expansion_failed", "error": str(error)})
        return

    carousel = build_carousel(complete_idea)
    if not carousel:
        print("Carousel structure is invalid.")
        save_output({"status": "skipped", "reason": "invalid_carousel"})
        return

    save_output({"status": "ready", "carousel": carousel})
