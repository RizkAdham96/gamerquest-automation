...
class TestSocialAIRunner(unittest.TestCase):
    ...
    def test_expand_idea_generates_complete_carousel_package(self,mock_call):
        ...
        result=idea_generator.expand_idea({"topic":"Topic","angle":"Angle","format":"ranking","hook":"Hook","total_score":82},[{"title":"Article","excerpt":"Facts","source_type":"news"}])
        self.assertEqual(len(result["slides"]),5); self.assertEqual(result["total_score"],82)
    ...
    def test_verify_carousel_rejects_unsupported_claims(self,mock_call):
        ...
        result=idea_generator.verify_carousel({"topic":"Game","slides":[]},[{"title":"Game","excerpt":"Solo","source_type":"news"}])
        self.assertFalse(result["valid"]); self.assertIn("cross-platform multiplayer",result["unsupported_claims"])
    ...
    def test_repair_carousel_changes_only_package_fields(self,mock_call):
        ...
        fixed=idea_generator.repair_carousel(original,[{"title":"Game","excerpt":"Supported fact","source_type":"deal"}],["Unsupported claim"])
        self.assertEqual(fixed["topic"],"Game"); self.assertEqual(fixed["total_score"],80.5); self.assertEqual(fixed["slides"][0]["body"],"Supported fact")
